...
def concat_cmaps(
    ...
    """
    Concatenate multiple colormaps into one, dividing the output colormap space equally among inputs.
...
def concat_palettes(
    ...
    """
    Concatenate multiple color palettes into one, preserving the order of colors.
    ...
    for palette in palettes:
        # Add its colors to the concatenated 
...
def create_diverging_cmap(
    ...
    """
    # # Resample both colormaps to ensure they have the same number of colors
    # left = left_cmap._resample(128)
    # right = right_cmap._resample(128)
   
    # Combine the two colormaps into one diverging colormap
    cmap = np.vstack((
