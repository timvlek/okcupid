...
def darken_color(
    ...
    Parameters:
    color (tuple): The color to be darkened in RGBA format (R, G, B, A).
    factor (float): The factor to darken the color by, default is 0.3.

    Returns:
    tuple[int, ...]: A tuple representing the darker color in RGBA format (R, G, B, A).
    """

    return tuple(rgba * (1 - factor) for rgba in color)
...
def boxplot(
    ...
    *args: Positional arguments passed to sns.boxplot().
    grid (str | None): Specifies the grid axis ('x', 'y', 'both') or None to disable grid.
    spines (list[str]): List of spines to show ('left', 'right', 'bottom', 'top').
    ticks (str | bool): Specifies the tick marks ('both', 'x', 'y') or False to disable ticks.
    xlabel (str): Label for the x-axis.
    ylabel (str): Label for the y-axis.
    xlim (tuple[float, float]] | None): Limit for the x-axis.
    ylim (tuple[float, float]] | None): Limit for the y-axis.
    **kwargs: Additional keyword arguments passed to sns.boxplot().
    ...
    if 'order' not in kwargs:
        kwargs['order'] = data.groupby(y)[x].median(
